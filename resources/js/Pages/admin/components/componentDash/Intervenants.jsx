...
function Toast({ msg, type, onClose }) {
    ...
    return (
        ...
        >
            <i className={`fa-solid ${type === "success" ? "fa-circle-check" : type === "error" ? "fa-circle-xmark" : "fa-circle-info"}`} style={{ color: c, fontSize: 16 }} />
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 4 }}>
                ...
                >
                    {loading
                        ...
                        : <><i className="fa-solid fa-user-plus" /> {Object.keys(initial).length > 1 ? "Mettre à jour" : "Ajouter"}</>
                    }
                </button>
            </div>
        </form>
...
function VilleChart({ data }) {
    ...
    if (!data.length) return <p style={{ fontSize: "0.8rem", color: t.textFaint, textAlign: "center", padding: "20px 0" }}>Aucune donnée</p>;
            ...
            <div style={{ background: t.bg, borderBottom: `1px solid ${t.border}`, padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
                <div style={{ flex: 1 }}>
                    <h1 style={{ margin: 0, fontSize: "1.2rem", fontWeight: 800, color: t.text, display: "flex", alignItems: "center", gap: 10 }}>
                        <i className="fa-solid fa-user-tie" style={{ color: PRIMARY }} /> Intervenants
                    </h1>
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 12px", background: t.bgInput, borderRadius: 20, border: `1px solid ${t.borderMd}` }}>
                    ...
                    <span style={{ fontSize: "0.75rem", color: t.text, fontWeight: 700 }}>Intervenants</span>
                </div>

                <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, justifyContent: "flex-end" }}>
                    <div style={{ display: "flex", background: t.bgInput, borderRadius: 8, padding: 2, border: `1px solid ${t.borderMd}` }}>
                        {[{id: "liste", icon: "fa-list"}, {id: "stats", icon: "fa-chart-pie"}].map((tab) => (
                            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
                                border: "none", background: activeTab === tab.id ? PRIMARY : "transparent",
                                color: activeTab === tab.id ? "#fff" : t.textSub, padding: "6px 12px",
                    ...
                    <button
                        ...
                        style={{
                            ...
                            padding: "8px 16px", fontWeight: 700, cursor: "pointer", fontSize: "0.8rem",
                            boxShadow: "0 4px 12px rgba(217,119,6,0.2)", transition: "transform 0.15s",
                    ...
                    <>
                        ...
                        <div style={{ marginBottom: 20 }}>
                            <div style={{
                                ...
                                background: t.bgInput, border: `1px solid ${cinSearch ? PRIMARY : t.borderMd}`,
                                borderRadius: 8, padding: "2px 6px", width: 240,
                                boxShadow: cinSearch ? "0 0 0 3px rgba(217,119,6,0.12)" : "none",
                                transition: "all 0.25s",
                            }}>
                                <i className="fa-solid fa-magnifying-glass" style={{ color: cinSearch ? PRIMARY : t.textMuted, fontSize: 11 }} />
                                <input
                                    value={cinSearch}
                                    ...
                                    style={{
                                        ...
                                        color: t.text, fontSize: "0.78rem", fontFamily: "'DM Sans', sans-serif",
                                        width: "100%",
                                    }}
                                />
                                {cinSearch && (
                        ...
                        {loading ? (
                            ...
                            <div style={{ textAlign: "center", padding: "60px 20px", background: t.bg, borderRadius: 12, border: `1px solid ${t.border}` }}>
                                ...
                                <p style={{ color: t.textMuted, margin: 0, fontSize: "0.9rem" }}>
                                    {cinSearch ? `Aucun intervenant avec CIN "${cinSearch}"` : "Aucun intervenant enregistré"}
                                </p>
                                {!cinSearch && (
                            ...
                            <div style={{ background: t.bg, borderRadius: 12, border: `1px solid ${t.border}`, boxShadow: t.shadow, overflow: "hidden" }}>
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 1.2fr 1.2fr 1.4fr auto", padding: "10px 18px", borderBottom: `1px solid ${t.border}`, background: t.bgAlt }}>
                                    {["Intervenant", "CIN", "Téléphone", "Ville", "Organisation", ""].map((h, i) => (
                                ...
                                {filtered.map((inv, idx) => (
                                    <div key={inv.id_inter}>
                                        ...
                                        >
                                            ...
                                            {/* Org */}
                                            <div>
                                                <Chip label={inv.organisation?.nom || "—"} color="#3b82f6" />
                                            </div>
                                            {/* Actions */}
                                        ...
                                        {expandedId === inv.id_inter && (
                                            <div style={{ background: t.dark ? "rgba(255,255,255,0.02)" : "#faf9f7", padding: "14px 18px 18px 68px", borderBottom: idx < filtered.length - 1 ? `1px solid ${t.border}` : "none", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
                                                {[
                                                    ...
                                                    { label: "Mission", value: inv.mission, icon: "fa-crosshairs" },
                                                    { label: "Nationalité", value: inv.nationalite, icon: "fa-flag" },
                                                    ...
                                                    <div key={item.label}>
                                                        ...
                                                        <div style={{ fontSize: "0.82rem", color: t.textSub }}>{item.value || "—"}</div>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                        ...
                        {loading || !stats ? (
                            ...
                            <>
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16, marginBottom: 24 }}>
                                    ...
                                    <StatCard icon="fa-solid fa-city" label="Villes représentées" value={stats.by_ville.length} color="#3b82f6" sub="villes distinctes" />
                                    <StatCard icon="fa-solid fa-flag" label="Nationalités" value={stats.by_nationalite.length} color="#8b5cf6" sub="nationalités distinctes" />
